$(document).ready(function(){
    ...
    function loadImages(){
        ...
        var length =  Math.floor(($("#container").width()/img_size) * ($("#container").height()/img_size));

        for(var i = 0; i < length; i++){
            images += "<img id='img_"+i+"' src='" + img_array[i] + "' data-sl='"+i+"' alt=''>";          
        }
    ...
    function swipeImagesTopBottom(i, j){
        ...
        if(!$("*[data-sl="+i+"]").offset().top || !$("*[data-sl="+j+"]").offset().top){
            console.log("Error Handled"+"\nOffset 1: "+$("*[data-sl="+i+"]").offset()+"\nOffset 2: "+$("*[data-sl="+j+"]").offset());
            return;
        }
        ...
        if((el_1.top != undefined || el_2.top != undefined) && (el_1.left == el_2.left) && (el_1.top < el_2.top)){
            $("*[data-sl="+el_1.sl+"]").animate({top: '+='+img_size+"px"}, 555);
            $("*[data-sl="+el_2.sl+"]").animate({top: '-='+img_size+"px"}, 555);

            $("#"+el_1.id).attr('data-sl', el_2.sl);
            $("#"+el_2.id).attr('data-sl', el_1.sl);
        }
        else if((el_1.top != undefined || el_2.top != undefined) && (el_1.left == el_2.left) && (el_1.top > el_2.top)){
            $("*[data-sl='"+el_1.sl+"']").animate({top: '-='+img_size+"px"}, 555);
            $("*[data-sl='"+el_2.sl+"']").animate({top: '+='+img_size+"px"}, 555);
    ...
    function swipeImagesRightLeft(i, j){
        ...
        if((el_1.top != undefined || el_2.top != undefined) && (el_1.left < el_2.left) && (el_1.top == el_2.top)){
            $("*[data-sl="+el_1.sl+"]").animate({left: '+='+img_size+"px"}, 555);
            $("*[data-sl="+el_2.sl+"]").animate({left: '-='+img_size+"px"}, 555);
            ...
            var image_per_horizontal_line =  Math.floor($("#container").width()/img_size);

            if(i > 0 && (i/image_per_horizontal_line) >= 1.90){
    ...
    function startGiggleImages(){
        interval_id = setInterval(function(){ 
            ...
            if(option_picker == "<>"){
                ...
                if(img_random_sl_picker > 0 && (img_random_sl_picker/image_per_horizontal_line) >= 1.90){
                    swipeImagesTopBottom(img_random_sl_picker, img_random_sl_picker-image_per_horizontal_line);
                }
                else{
                    swipeImagesTopBottom(img_random_sl_picker, img_random_sl_picker+image_per_horizontal_line);
                }
